...
use crate::upload::{UploadState, delete_video, get_video, list_videos, publish_concat, upload_video};

fn resolve_path_from_monorepo_root(path: &std::path::Path) -> anyhow::Result<PathBuf> {
	if path.is_absolute() {
	...
	let repo_root = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
		.ancestors()
		.nth(2)
		.ok_or_else(|| anyhow::anyhow!("cannot resolve repo root from CARGO_MANIFEST_DIR"))?;
...
pub struct UploadConfig {
	#[arg(long = "web-upload", env = "MOQ_WEB_UPLOAD", default_value = "true")]
	pub enabled: bool,

	#[arg(long = "web-upload-db", env = "MOQ_WEB_UPLOAD_DB", default_value = "./videos.sqlite")]
	pub db_path: PathBuf,

	#[arg(long = "web-upload-dir", env = "MOQ_WEB_UPLOAD_DIR", default_value = "./uploads")]
	pub upload_dir: PathBuf,

	#[arg(long = "web-media-dir", env = "MOQ_WEB_MEDIA_DIR", default_value = "demo/pub/media")]
	pub media_dir: PathBuf,

	#[arg(long = "web-media-list", env = "MOQ_WEB_MEDIA_LIST", default_value = "demo/pub/media/list.txt")]
...
impl Default for UploadConfig {
	fn default() -> Self {
		Self {
			enabled: true,
			db_path: PathBuf::from("./videos.sqlite"),
			upload_dir: PathBuf::from("./uploads"),
			media_dir: PathBuf::from("demo/pub/media"),
			list_path: PathBuf::from("demo/pub/media/list.txt"),
		}
	}
}
...
impl Web {
	...
	pub async fn run(self) -> anyhow::Result<()> {
		...
		if self.config.upload.enabled {
			tokio::fs::create_dir_all(&self.config.upload.upload_dir).await?;

			let media_dir = resolve_path_from_monorepo_root(&self.config.upload.media_dir)?;
			let list_path = resolve_path_from_monorepo_root(&self.config.upload.list_path)?;

			tokio::fs::create_dir_all(&media_dir).await?;

			if let Some(parent) = self.config.upload.db_path.parent() {
			...
			let options = SqliteConnectOptions::new()
				.filename(&self.config.upload.db_path)
				.create_if_missing(true);
			let pool = SqlitePoolOptions::new()
				.max_connections(10)
				.connect_with(options)
				.await?;
			crate::upload::init_db(&pool).await?;

			let upload_state = UploadState {
				pool,
				upload_dir_abs: tokio::fs::canonicalize(&self.config.upload.upload_dir).await?,
				media_dir_abs: tokio::fs::canonicalize(&media_dir).await?,
				list_path_abs: tokio::fs::canonicalize(&list_path).await?,
			};
			state.upload = Some(upload_state);
			...
			app = app
				...
				.route("/api/publish/{channel}", post(publish_concat));
		}
		...
		let app = app
			.layer(CorsLayer::new().allow_origin(Any).allow_methods([Method::GET, Method::POST, Method::DELETE]))
